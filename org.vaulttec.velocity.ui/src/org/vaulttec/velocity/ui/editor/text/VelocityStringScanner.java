...
public class VelocityStringScanner extends RuleBasedScanner {
	...
	public VelocityStringScanner(VelocityColorProvider colorProvider) {
		...
		Token token = new Token(new TextAttribute(colorProvider.getColor(IColorConstants.STRING_REFERENCE)));
		rules.add(new PatternRule("$!{", "}", token, (char) 0, true));
		rules.add(new PatternRule("${", "}", token, (char) 0, true));

		// Add pattern rule for shorthand references
		token = new Token(new TextAttribute(colorProvider.getColor(IColorConstants.STRING_REFERENCE)));
		...
		setDefaultReturnToken(new Token(new TextAttribute(colorProvider.getColor(IColorConstants.STRING))));
	}
